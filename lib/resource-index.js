...
export async function loadWebflowSnapshot({
  siteId,
  token,
  fetchImpl = fetch,
  deadline = Infinity,
}) {
  const options = { token, fetchImpl, deadline };
...
export async function buildResourceIndex(
  ...
  const imageEnrichment = imageFetchImpl
    ? await enrichResponsiveImages(items, {
        fetchImpl: imageFetchImpl,
        deadline: imageDeadline,
        cachedItems,
      })
    : { enriched: 0, reused: 0, failures: [] };
